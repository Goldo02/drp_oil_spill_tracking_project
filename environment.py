...
class SimulationMap:
    ...
    def __init__(
        ...
        self.x_coords = np.linspace(self.xlim[0], self.xlim[1], self.grid_size)
        self.y_coords = np.linspace(self.ylim[0], self.ylim[1], self.grid_size)
...
class OilSpill(Field):
    ...
    def get_field(self, X=None, Y=None):
        ...
        raise ValueError("No cached field is available. Provide X and Y.")
...
class CircleOilSpill(OilSpill):
    ...
    def field(self, X, Y):
        ...
        return np.where(
            ...
            1.0,
            np.exp(-((distance - self.radius) ** 2) / (2.0 * self.sigma**2)),
        )
...
class SmoothedPolygonOilSpill(OilSpill):
    ...
    def _generate_vertices(self):
        ...
        harmonic = 0.55 * np.sin(3.0 * angles + phase_1) + 0.25 * np.sin(
            5.0 * angles + phase_2
        )
    ...
    def _catmull_rom_closed(points, samples):
        ...
        padded = np.vstack([points[-1], points, points[0], points[1]])
